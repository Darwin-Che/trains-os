...
static inline bool cmp_keys_task_dsp(struct kPriorityKey key1, struct kPriorityKey key2)
{
...
static inline void k_sched_add_ready(struct kScheduler *sched, struct kTaskDsp *td)
{
  td->state = READY;
  HEAP_INSERT(kTaskDspHeap, &sched->task_heap, priority_key, td);
}

static inline void k_sched_add_reply_wait(struct kScheduler *sched, struct kTaskDsp *td)
{
  DL_APPEND2(sched->reply_blocked, td, prev_sched, next_sched);
  td->state = REPLY_BLOCKED;
}

static inline void k_sched_remove_reply_wait(struct kScheduler *sched, struct kTaskDsp *td)
{
  assert(td->state == REPLY_BLOCKED);
  DL_DELETE2(sched->reply_blocked, td, prev_sched, next_sched);
}

static inline void k_sched_add_send_wait(struct kScheduler *sched, struct kTaskDsp *td)
{
  DL_APPEND2(sched->send_blocked, td, prev_sched, next_sched);
  td->state = SEND_BLOCKED;
}

static inline void k_sched_remove_send_wait(struct kScheduler *sched, struct kTaskDsp *td)
{
  assert(td->state == SEND_BLOCKED);
  DL_DELETE2(sched->send_blocked, td, prev_sched, next_sched);
}

static inline void k_sched_add_recv_wait(struct kScheduler *sched, struct kTaskDsp *td)
{
  DL_APPEND2(sched->recv_blocked, td, prev_sched, next_sched);
  td->state = RECEIVE_BLOCKED;
}

static inline void k_sched_remove_recv_wait(struct kScheduler *sched, struct kTaskDsp *td)
{
  assert(td->state == RECEIVE_BLOCKED);
  DL_DELETE2(sched->recv_blocked, td, prev_sched, next_sched);
}

static inline void k_sched_add_event_wait(struct kScheduler *sched, struct kTaskDsp *td, enum keIntrId intr_id)
{
  DL_APPEND2(sched->event_blocked, td, prev_sched, next_sched);
  td->state = (enum kTaskState)intr_id;
}

static inline void k_sched_remove_event_wait(struct kScheduler *sched, struct kTaskDsp *td)
{
